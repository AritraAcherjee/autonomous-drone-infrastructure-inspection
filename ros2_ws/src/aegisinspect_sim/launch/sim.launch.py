"""Start the fixed inspection bay and one-way sensor/clock bridge."""
...
def _start(context):
    share = Path(get_package_share_directory('aegisinspect_sim'))
    headless = LaunchConfiguration('headless').perform(context).lower() == 'true'
    args = ['-r', '-v', '3']
    if headless:
        args += ['-s', '--headless-rendering']
    args.append(str(share / 'worlds' / 'inspection_bay.sdf'))
    return [
        ...
        IncludeLaunchDescription(
            ...
            launch_arguments={'gz_args': shlex.join(args), 'on_exit_shutdown': 'true'}.items(),
        ),
        Node(
            package='ros_gz_bridge', executable='parameter_bridge',
            name='sensor_bridge', namespace='/aegis/sim', output='screen',
            parameters=[{
